...
def _upload_image(supabase_url: str) -> str | None:
    """Upload the latest generated image to Supabase Storage.

    Reads output/latest_image_path.txt (written by create_post_image_gemini)
    to find the unique filename for this run. Uploads it with that unique name
    so each news post gets its own image — no more overwrites.
    """
    if not _LATEST_IMAGE_FILE.exists():
        print("[save_to_supabase] No latest_image_path.txt — skipping image upload.")
        return None

    image_path = Path(_LATEST_IMAGE_FILE.read_text(encoding="utf-8").strip())
...
def save_posts_to_supabase(social_posts_markdown: str) -> str:
    ...
    def _upload_in_thread():
        try:
            _result_box.append(_upload_image(supabase_url))
